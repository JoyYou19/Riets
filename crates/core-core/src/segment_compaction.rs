...
impl SegmentCompactionWorker {
    pub fn start(sender: Sender<ShardCmd>, dead_ratio_threshold: f64, interval: Duration) -> Self {
        ...
        let handle = thread::spawn(move || {
            while !stop_thread.load(Ordering::Relaxed) {
                ...
                sender
                    .send(ShardCmd::PlanSegmentCompaction { dead_ratio_threshold, reply })
                    .map_err(|_|
                    ...
                    Ok(Ok(Some(job))) => {
                        ...
                        match run_segment_compaction(job) {
                            Ok(completed) => {
                                ...
                                match
                                    sender.send(ShardCmd::InstallSegmentCompaction {
                                        completed,
                                        ack: Some(ack),
                                    })
                                {
                                    Ok(()) =>
                                        match install_rx.recv() {
                                            Ok(Ok(installed)) =>
                                                eprintln!(
                                                    "[segcompact] install result: {installed}"
                                                ),
                                            Ok(Err(e)) =>
                                                eprintln!("[segcompact] install error: {e}"),
                                            Err(_) =>
                                                eprintln!(
                                                    "[segcompact] install ack channel dropped"
                                                ),
                                        }
                                    Err(_) =>
                                        eprintln!(
    ...
    pub fn stop(mut self) -> io::Result<()> {
        ...
        if let Some(h) = self.handle.take() {
            h.join().map_err(|_| io::Error::other("segment compaction worker panicked"))??;
        }
        Ok(())
    }
}
